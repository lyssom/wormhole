...
func (b *BitBlockCounter) getBlockSlow(blockSize int64) BitBlockCount {
	...
	popcnt := int16(CountSetBits(b.bitmap, int(b.bitOffset), int(runlen)))
...
func (b *BitBlockCounter) NextFourWords() BitBlockCount {
	...
	b.bitmap = b.bitmap[BytesForBits(fourWordsBits):]
...
func VisitBitBlocks(bitmap []byte, offset, length int64, visitValid func(pos int64), visitInvalid func()) {
	...
	for pos < length {
		...
		if block.AllSet() {
			...
			for i := 0; i < int(block.Len); i, pos = i+1, pos+1 {
				if BitIsSet(bitmap, int(offset+pos)) {
					visitValid(pos)
				} else {
					visitInvalid()
				}
...
func VisitTwoBitBlocks(leftBitmap, rightBitmap []byte, leftOffset, rightOffset int64, len int64, visitValid func(pos int64), visitNull func()) {
	...
	for pos < len {
		...
		if block.AllSet() {
			...
			for i := 0; i < int(block.Len); i, pos = i+1, pos+1 {
				if BitIsSet(leftBitmap, int(leftOffset+pos)) && BitIsSet(rightBitmap, int(rightOffset+pos)) {
					visitValid(pos)
				} else {
					visitNull()
				}
...
func (b *BinaryBitBlockCounter) nextWord(op bbitOp) BitBlockCount {
	if b.bitsRemaining == 0 {
	...
	if b.bitsRemaining < b.bitsRequiredForWords {
		...
		for i := int16(0); i < runLength; i++ {
			if op.bit(BitIsSet(b.left, int(b.leftOffset)+int(i)),
				BitIsSet(b.right, int(b.rightOffset)+int(i))) {
				popcount++
			}
		}
