...
func NewBitRunReader(bitmap []byte, offset int64, length int64) BitRunReader {
	...
	ret.curRunBitSet = BitIsNotSet(bitmap, int(offset))
...
func (b *bitRunReader) advanceUntilChange() {
	newbits := int64(0)
	for {
		b.bitmap = b.bitmap[int(unsafe.Sizeof(uint64(0))):]
		b.loadNextWord()
...
func (b *bitRunReader) loadWord(bitsRemaining int64) {
	...
	if bitsRemaining >= 64 {
		...
		nbytes := BytesForBits(bitsRemaining)
		wordptr := (*(*[8]byte)(unsafe.Pointer(&b.word)))[:]
		copy(wordptr, b.bitmap[:nbytes])

		SetBitTo(wordptr, int(bitsRemaining), BitIsNotSet(wordptr, int(bitsRemaining-1)))
