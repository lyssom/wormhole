...
import (
	...
	arrow "wormhole/server/engine/parquet/internal/types"
)
...
func CastToBytes32(b []int32) []byte {
...
func CastToBytes64(b []int64) []byte {
...
// CastToBytes reinterprets the slice b to a slice of bytes.
func CastToBytesf32(b []float32) []byte {
...
// CastToBytes reinterprets the slice b to a slice of bytes.
func CastToBytesf64(b []float64) []byte {
...
func init() {
	...
	if endian.IsBigEndian {
		...
		writeInt32LE = func(e *encoder, in []int32) {
			e.append(CastToBytes32(in))
		}
		copyFromInt32LE = func(dst []int32, src []byte) {
			copy(CastToBytes32(dst), src)
		}
		writeInt64LE = func(e *encoder, in []int64) {
			e.append(CastToBytes64(in))
		}
		copyFromInt64LE = func(dst []int64, src []byte) {
			copy(CastToBytes64(dst), src)
		}
		writeFloat32LE = func(e *encoder, in []float32) {
			e.append(CastToBytesf32(in))
		}
		copyFromFloat32LE = func(dst []float32, src []byte) {
			copy(CastToBytesf32(dst), src)
		}
		writeFloat64LE = func(e *encoder, in []float64) {
			e.append(CastToBytesf64(in))
		}
		copyFromFloat64LE = func(dst []float64, src []byte) {
			copy(CastToBytesf64(dst), src)
		}
	}
}
...
func (dec *PlainInt32Decoder) DecodeSpaced(out []int32, nullCount int, validBits []byte, validBitsOffset int64) (int, error) {
	...
	if dec.bitSetReader == nil {
		dec.bitSetReader = bitutil.NewReverseSetBitRunReader(validBits, validBitsOffset, int64(nvalues))
	} else {
