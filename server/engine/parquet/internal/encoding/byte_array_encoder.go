...
type PlainByteArrayEncoder struct {
	...
	bitSetReader bitutil.SetBitRunReader
}
...
func (enc *PlainByteArrayEncoder) PutByteArray(val parquet.ByteArray) {
	inc := val.Len() + int(unsafe.Sizeof(uint32(0)))
...
func (enc *PlainByteArrayEncoder) PutSpaced(in []parquet.ByteArray, validBits []byte, validBitsOffset int64) {
	if validBits != nil {
		if enc.bitSetReader == nil {
			enc.bitSetReader = bitutil.NewSetBitRunReader(validBits, validBitsOffset, int64(len(in)))
		} else {
...
// 		out = out[int(unsafe.Sizeof(uint32(0))):]
// 		copy(out, v)
// 		out = out[len(v):]
// 	})
// }
...
// 	}
// 	if !found {
// 		enc.dictEncodedSize += in.Len() + int(unsafe.Sizeof(uint32(0)))
// 	}
