...
import (
	...
	"wormhole/server/engine/parquet/common/bitutil"

	"wormhole/server/engine/parquet/common/utils"
...
func (r *RleDecoder) GetBatchSpaced(vals []uint64, nullcount int, validBits []byte, validBitsOffset int64) (int, error) {
	...
	blockCounter := bitutil.NewBitBlockCounter(validBits, validBitsOffset, int64(len(vals)))

	var (
		totalProcessed int
		processed      int
		block          bitutil.BitBlockCount
		err            error
	)
...
func (r *RleDecoder) getspacedUint64(dc DictionaryConverter, vals []uint64, batchSize, nullCount int, validBits []byte, validBitsOffset int64) (int, error) {
	...
	bitReader := bitutil.NewBitRunReader(validBits, validBitsOffset, int64(batchSize))
...
func (r *RleDecoder) consumeRepeatCounts(read, batchSize, remain int, run bitutil.BitRun, bitRdr bitutil.BitRunReader) (int, int, bitutil.BitRun) {
...
func (r *RleDecoder) consumeLiteralsUint64(dc DictionaryConverter, vals []uint64, remain int, buf []IndexType, run bitutil.BitRun, bitRdr bitutil.BitRunReader) (int, int, bitutil.BitRun, error) {
